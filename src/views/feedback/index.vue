  ...
  <div>
    <el-card>
      <div class="searchDiv">
        ...
        <el-input type="text" placeholder="用户ID/用户名/反馈内容/注册手机号" class="width1 width2" v-model="sch_order"></el-input>
      ...
      <el-table :data="tableData" :header-cell-style="{ background: '#eaf6f6', color: '#13b5b1' }" border style="box-shadow:0 0 10px #e2e2e2">
        ...
        <el-table-column prop="content" align="center" label="反馈内容" width="320px"></el-table-column>
        <el-table-column prop="contact" align="center" label="联系方式"></el-table-column>
        <el-table-column prop="createDateTime" align="center" label="反馈时间" width="200"></el-table-column>
        <el-table-column prop="status" align="center" label="处理状态" width="100px">
          <template slot-scope="scope">
            {{ scope.row.status | changeStatu }}
          </template>
        </el-table-column>
        <el-table-column prop="disposeCount" align="center" label="处理记录" width="100px">
          <template slot-scope="scope">
            <span @click="handleCode(scope.row.feedBackId)" class="companyColor point" v-show="scope.row.disposeCount > 0">{{ scope.row.disposeCount }}</span>
          </template>
        </el-table-column>
        <el-table-column align="center" label="操作">
          <template slot-scope="scope">
            <el-button type="warning" size="small" @click="handleStatu(scope.row.feedBackId, scope.$index)">标记处理</el-button>
          </template>
        </el-table-column>
      </el-table>
